...
void makeLL1MBDHist::Loop()
{
   if (fChain_mbd == 0 || fChain_ll1 == 0) return;
   
   Long64_t nentries = min(fChain_ll1->GetEntriesFast(), fChain_mbd->GetEntriesFast());
   ...
   while (jentry<nentries) {
      ...
      for (int ic = 0; ic < 64; ic++)
	{
      ...
      if (trig[54][idxem] >= thresh && trig[55][idxem] >= thresh)bad++;
      else if (!(nhit_n[id] < thresh && nhit_s[id] < thresh)) cout << "nothing? "<< dec <<ll1evtnr << " "<<endl;;
      nen++;
   }
...
void makeLL1MBDHist::One(int event = 10)
{
   if (fChain_mbd == 0 || fChain_ll1 == 0) return;
   ...
   for (Long64_t jentry=event; jentry<event+1;jentry++) {
      ...
      nbll1 = fChain_ll1->GetEntry(jentry);   nbytesll1 += nbll1;
      if (Cut(ientry) < 0) continue;
      while (femevtnr[0] != ll1evtnr)
	{
	  jentry++;
      ...
      for (int ic = 0; ic < 8; ic++)
	{
      ...
      EmulateTrigger(adc, trig, hitmap);
      cout <<"Emulator: "<<endl;
      cout << dec <<"evt ll1/mbd: "<< ll1evtnr << " / "<<femevtnr[0]<<endl;
      cout <<"FEMS: ";
